...
public:
  ...
  inline unsigned long long count() const { return this->_count.load(); }
};
 ...
 *  2) The use of specific strings to check that a count_down should occur
 *  3) The ability to reuse the latch(may cause problems in non-trivial cases)              
 */
...
public:
  ...
  inline bool try_lock() { return TryEnterCriticalSection(&_cs); }
