	...
	void init() 
	{
		...
		for (auto & i : _itemNames)	
		{
			_rawBlock.insert(
				_rawBlockElemTy(
					i, 
					std::move( populateTBlock( std::move(_tRowPtrTy(new _tRowTy)) )) 
					)
				); 
		}
	}
...
public:
		...
		) 
	{
		if( sItems.empty() || tTopics.empty() )	
			throw TOSDB_data_block_error("RawDataBlock<>::CreateBlock(...) : sItems and tTopics can not be empty");			
	...
	{
		return (mbc >= _blockCount_) ? (_maxBlockCount_ = mbc) : _maxBlockCount_ ; 
	}
	...
	void insertData( TOS_Topics::TOPICS tTopic, std::string sItem, Val val, DT&& datetime ); 
...
template< typename T, typename T2 > size_type	
RawDataBlock<T,T2>::_blockCount_ = 0;

template< typename T, typename T2 > size_type	
RawDataBlock< T, T2>::_maxBlockCount_ = 10;
...
void RawDataBlock<TG,TD>::insertData( TOS_Topics::TOPICS tTopic, std::string sItem, Val val, DT&& datetime ) 
{
	...
	try {		
		...
		if ( !tBlck ) 
			throw TOSDB_data_block_error("insertData() _rawBlock returned nullptr for this item");	
		dBlck = (tBlck->at( tTopic )).get();
		if ( !dBlck ) 
			throw TOSDB_data_block_error("insertData() _rawBlock returned nullptr for this topic");			
...
template< typename TG, typename TD > void 
RawDataBlock<TG,TD>::add_item( std::string sItem ) 
{		
	try
	{
		_guardTy _lock_(*_mtxR);
...
template< typename TG, typename TD > void 
RawDataBlock<TG,TD>::remove_item( std::string sItem )
{	
	try
	{
		_guardTy _lock_(*_mtxR);
	...
	{
		TOSDB_LogH("DataBlock", "remove_item(): out_of_range exception; probably invalid item parameter");
...
template< typename TG, typename TD > void
RawDataBlock<TG,TD>::add_topic( TOS_Topics::TOPICS tTopic) 
{	
	try
	{
		_guardTy _lock_(*_mtxR);
...
template< typename TG, typename TD > void 
RawDataBlock<TG,TD>::remove_topic( TOS_Topics::TOPICS tTopic)
{	
	try 
	{
		_guardTy _lock_(*_mtxR);
	...
	{
		TOSDB_LogH("DataBlock", "remove_topic(): out_of_range exception; probably invalid topic parameter");
		throw TOSDB_data_block_error(e, "remove_item() probably invalid topic ");		
	}
	catch( const std::exception & e )
	{
...
template< typename TG, typename TD > typename RawDataBlock<TG,TD>::_tRowTy* 
RawDataBlock<TG,TD>::insertTopic( typename RawDataBlock<TG,TD>::_tRowTy* tBlock, TOS_Topics::TOPICS topic)
{		
	...
	{ 
		case TOSDB_STRING_BIT :
			dataBlock = this->_dtFlag ? 
				new tosdb_data_stream::Object< std::string, datetime_type, generic_type, true >(_blockSize) :
				new tosdb_data_stream::Object< std::string, datetime_type, generic_type, false >(_blockSize);
			break;
		case TOSDB_INTGR_BIT :
			dataBlock = this->_dtFlag ? 
				new tosdb_data_stream::Object< def_size_type, datetime_type, generic_type, true >(_blockSize) :
				new tosdb_data_stream::Object< def_size_type, datetime_type, generic_type, false >(_blockSize);
			break;
		case TOSDB_QUAD_BIT :
			dataBlock = this->_dtFlag ? 
				new tosdb_data_stream::Object< ext_price_type, datetime_type, generic_type, true >(_blockSize) :
				new tosdb_data_stream::Object< ext_price_type, datetime_type, generic_type, false >(_blockSize);
			break;
		case TOSDB_INTGR_BIT | TOSDB_QUAD_BIT :
			dataBlock = this->_dtFlag ? 
				new tosdb_data_stream::Object< ext_size_type, datetime_type, generic_type, true >(_blockSize) :
				new tosdb_data_stream::Object< ext_size_type, datetime_type, generic_type, false >(_blockSize);
			break;
		default :
			dataBlock = this->_dtFlag ? 
				new tosdb_data_stream::Object< def_price_type, datetime_type, generic_type, true >(_blockSize) :
				new tosdb_data_stream::Object< def_price_type, datetime_type, generic_type, false >(_blockSize);
	}
	tBlock->erase(topic);
	try
	{
		tBlock->insert( 
			_tRowElemTy( topic, _iStreamPtrTy( std::move(dataBlock) ) ) 
			);
	}
	catch( ... )
	{
...
template< typename TG, typename TD > typename RawDataBlock<TG,TD>::_tRowPtrTy 
...
template< typename TG, typename TD > const typename RawDataBlock<TG,TD>::_iStreamTy*
RawDataBlock<TG,TD>::raw_stream_ptr( std::string sItem, TOS_Topics::TOPICS tTopic ) const 
{
	...
	{
		TOSDB_LogH("DataBlock", "raw_stream_ptr(): out_of_range exception; probably invalid item and/or topic");
		throw TOSDB_data_block_error(e, "DataStream does not exist in block; probably invalid item and/or topic");
	}
	catch(const std::exception & e)
	{
...
template< typename TG, typename TD > typename RawDataBlock<TG,TD>::map_type
	...
	try
	{
		...
		if ( tBlck )
			for( auto & _P : *tBlck)
				map.insert( 
					std::move( pair_type(
						TOS_Topics::globalTopicMap[_P.first], 
						_P.second->operator[](0)
						) )
					);
	}
	catch( const std::out_of_range& e )
	{
		TOSDB_LogH("DataBlock", "map_of_frame_topics(): out_of_range exception; probably invalid item param");
		throw TOSDB_data_block_error(e, "map_of_frame_topics(); probably invalid item param");
	}
	catch( const tosdb_data_stream::error& e )
	{
		throw TOSDB_data_stream_error(e, "tosdb_data_stream error caught and encapsulated in RawDataBlock::map_of_frame_topics(...)" );
	}
	catch(const std::exception & e)
	{
...
RawDataBlock<TG,TD>::map_of_frame_items(TOS_Topics::TOPICS tTopic) const 
{
	...
	{
		_guardTy _lock_(*_mtxR);
		for( auto & _P : _rawBlock)
			map.insert( 
				std::move( pair_type(
					_P.first, 
					_P.second->at(tTopic)->operator[](0) 
					) )
				); 			
	}
	catch( const std::out_of_range& e )
	{
		TOSDB_LogH("DataBlock", "map_of_frame_items(): out_of_range exception; probably invalid topic param");
		throw TOSDB_data_block_error(e, "map_of_frame_items(); probably invalid topic");
	}
	catch( const tosdb_data_stream::error& e )
	{
		throw TOSDB_data_stream_error(e, "tosdb_data_stream error caught and encapsulated in RawDataBlock::map_of_frame_items(...)" );
	}
	catch( const std::exception & e )
	{
...
template< typename TG, typename TD > typename RawDataBlock<TG,TD>::map_datetime_type
RawDataBlock<TG,TD>::pair_map_of_frame_topics( std::string sItem ) const 
{
	...
	try
	{
		...
		if ( tBlck )
			for( auto & _P : *tBlck)
				map.insert( 
					std::move( map_datetime_type::value_type(
						TOS_Topics::globalTopicMap[_P.first], 
						_P.second->both(0)
						) )
					);
	...
	{
		TOSDB_LogH("DataBlock", "pair_map_of_frame_topics(): out_of_range exception; probably invalid item param");
		throw TOSDB_data_block_error(e, "pair_map_of_frame_topics(); probably invalid item");
	}
	catch( const tosdb_data_stream::error& e )
	{
		throw TOSDB_data_stream_error(e, "tosdb_data_stream error caught and encapsulated in RawDataBlock::pair_map_of_frame_topics(...)" );
	}
	catch(const std::exception & e)
	{
	...
	try
	{
		...
		for( auto & _P : _rawBlock)
			map.insert( 
				std::move( map_datetime_type::value_type(
					_P.first, 
					_P.second->at(tTopic)->both(0) 
					) )
				); 			
	}
	catch( const std::out_of_range& e )
	{
		TOSDB_LogH("DataBlock", "pair_map_of_frame_items(): out_of_range exception; probably invalid topic param");
		throw TOSDB_data_block_error(e, "pair_map_of_frame_items(); probably invalid topic");
	}
	catch( const tosdb_data_stream::error& e )
	{
		throw TOSDB_data_stream_error(e, "tosdb_data_stream error caught and encapsulated in RawDataBlock::pair_map_of_frame_items(...)" );
	}
	catch( const std::exception & e )
	{
...
template< typename TG, typename TD > typename RawDataBlock<TG,TD>::matrix_type
	...
	try
	{
		...
		{			
			...
			for(auto & tops : *items.second)
				map.insert( 
					std::move( map_type::value_type( 
						std::move( TOS_Topics::globalTopicMap[tops.first] ),
						tops.second->operator[](0) 
						) )
					); 
			matrix.insert( std::move( matrix_type::value_type( items.first, std::move(map) ) ) );
		}		
	}
	catch( const tosdb_data_stream::error& e )
	{
		throw TOSDB_data_stream_error(e, "tosdb_data_stream error caught and encapsulated in RawDataBlock::matrix_of_frame(...)" );
	}
	catch( const std::exception & e )
	{
...
template< typename TG, typename TD > typename RawDataBlock<TG,TD>::matrix_datetime_type
...
{ 
	...
	try
	{
		...
		{			
			...
			matrix.insert( std::move( matrix_datetime_type::value_type( items.first, std::move(map) ) ) );
		}		
	}
	catch( const tosdb_data_stream::error& e )
	{
		throw TOSDB_data_stream_error(e, "tosdb_data_stream error caught and encapsulated in RawDataBlock::pair_matrix_of_frame(...)" );
	}
	catch( const std::exception & e )
	{
		throw TOSDB_data_block_error(e, "matrix_of_frame()");
