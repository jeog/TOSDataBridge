...
/* name-conflict: we have a buffers_type in {}::client_admin.cpp */
...
public:
  ...
  DDE_Data(const TOS_Topics::TOPICS topic, 
    ...
    {
      if(datetime) _init_datetime();      
    }

  ~DDE_Data() { if(time) delete time; }
...
template<typename T> const system_clock_type::time_point  DDE_Data<T>::EPOCH_TP; 
