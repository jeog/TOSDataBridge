...
/*
int TOSDB_GetPreCachedTopicNames( LPCSTR id, LPSTR* dest, size_type strLen, size_type arrLen)
{
	const TOSDBlock *db;
	topic_set_type topS;
	int i, errVal;
...
int TOSDB_GetPreCachedItemNames( LPCSTR id, LPSTR* dest, size_type strLen, size_type arrLen )
{
	const TOSDBlock *db;
	str_set_type itemS;
	int i, errVal;
...
*/
int TOSDB_GetTopicNames( LPCSTR id, LPSTR* dest, size_type arrLen, size_type strLen )
{
	const TOSDBlock *db;
	topic_set_type topS;
	int i, errVal;
...
int TOSDB_GetItemNames( LPCSTR id, LPSTR* dest,  size_type arrLen,  size_type strLen )
{
	const TOSDBlock *db;
	str_set_type itemS;
	int i, errVal;
...
str_set_type TOSDB_GetTopicNames( std::string id )
{
	...
	return str_set_type(
			db->block->topics(), 
			[&]( TOS_Topics::TOPICS t ){ return TOS_Topics::globalTopicMap[ t ]; } 
			);
}
...
str_set_type TOSDB_GetPreCachedTopicNames( std::string id )
{
	...
	return str_set_type(
		db->topicPreCache,
		[=](TOS_Topics::TOPICS top){ return TOS_Topics::globalTopicMap[top]; } 
						);
}
...
int	TOSDB_GetStreamOccupancy( LPCSTR id,LPCSTR sItem, LPCSTR sTopic, size_type* sz )
{
	const TOSDBlock *db;
...
size_type TOSDB_GetStreamOccupancy( std::string id, std::string sItem, TOS_Topics::TOPICS tTopic )
{
	const TOSDBlock *db;	
	TOSDB_RawDataBlock::stream_const_ptr_type dat;
	rGuardTy _lock_(*globalMutex);
	...
	catch( const tosdb_data_stream::error& e )
	{
		throw TOSDB_data_stream_error( e, 
			"tosdb_data_stream error caught and encapsulated in TOSDB_GetStreamOccupancy()" 
			);
	}
	catch( ... )
	{
...
{	
	...
	{
		throw TOSDB_data_stream_error( e, 
			"tosdb_data_stream error caught and encapsulated in TOSDB_Get<generic_type,false>()" 
			);
	}
	catch( ... )
	{
...
TOSDB_Get< generic_type, true >( std::string id, std::string sItem, TOS_Topics::TOPICS tTopic, long indx )
{
	...
	{
		throw TOSDB_data_stream_error(e, 
			"tosdb_data_stream error caught and encapsulated in TOSDB_Get<generic_type,true>()" 
			);
	}
	catch( ... )
	{
...
TOSDB_Get_( std::string id, std::string sItem, TOS_Topics::TOPICS tTopic, long indx, T* dest, pDateTimeStamp datetime )
{ 	
	const TOSDBlock *db;
	TOSDB_RawDataBlock::stream_const_ptr_type dat;	
	try
...
template< typename T > int 
TOSDB_Get_( LPCSTR id, LPCSTR sItem, LPCSTR sTopic, long indx, T* dest, pDateTimeStamp datetime )
{ 
...
int 
TOSDB_GetDouble( LPCSTR id, LPCSTR sItem, LPCSTR sTopic, long indx, ext_price_type* dest, pDateTimeStamp datetime )
{	
	return TOSDB_Get_( id, sItem, sTopic , indx, dest, datetime );
}

int 
TOSDB_GetFloat( LPCSTR id, LPCSTR sItem, LPCSTR sTopic, long indx, def_price_type* dest, pDateTimeStamp datetime )
{
	return TOSDB_Get_( id, sItem, sTopic , indx, dest, datetime );
}

int 
TOSDB_GetLongLong( LPCSTR id, LPCSTR sItem, LPCSTR sTopic, long indx, ext_size_type* dest, pDateTimeStamp datetime )
{
	return TOSDB_Get_( id, sItem, sTopic , indx, dest, datetime );
}

int 
TOSDB_GetLong( LPCSTR id, LPCSTR sItem, LPCSTR sTopic, long indx, def_size_type* dest, pDateTimeStamp datetime )
{
	return TOSDB_Get_( id, sItem, sTopic , indx, dest, datetime );
}
int
TOSDB_GetString( LPCSTR id, LPCSTR sItem, LPCSTR sTopic, long indx, LPSTR dest, size_type strLen, pDateTimeStamp datetime )
{	
	const TOSDBlock *db;
	TOSDB_RawDataBlock::stream_const_ptr_type dat;
	TOS_Topics::TOPICS t;
...
TOSDB_GetStreamSnapshot<generic_type,false>( std::string id, std::string sItem, TOS_Topics::TOPICS tTopic, long end, long beg )
{
	...
	{
		throw TOSDB_data_stream_error(e, 
			"tosdb_data_stream error caught and encapsulated in TOSDB_GetStreamSnapshot<generic_type,false>()" 
			);
	}
	catch( ... )
	{
...
template<> std::pair< std::vector<generic_type>, std::vector<DateTimeStamp> > 		
TOSDB_GetStreamSnapshot<generic_type,true>( std::string id, std::string sItem, TOS_Topics::TOPICS tTopic, long end, long beg )
{
	...
	{
		throw TOSDB_data_stream_error(e, 
			"tosdb_data_stream error caught and encapsulated in TOSDB_GetStreamSnapshot<generic_type,true>()"
			);
	}
	catch( ... )
	{
...
struct GSSRetType< T, true>{
	...
	operator()( TOSDB_RawDataBlock::stream_const_ptr_type dat, long end, long beg, size_t diff )
	{				
		std::vector<T> tmpV(diff); //adjust for [ ) -> [ ] 
		std::vector< DateTimeStamp > tmpDTSV(diff); //adjust for [ ) -> [ ] 	
		if( diff > 0 )
			dat->copy( &(*(tmpV.begin())), diff, end, beg, &(*(tmpDTSV.begin())) );			
		return std::pair< std::vector<T>, std::vector<DateTimeStamp> >( tmpV, tmpDTSV );
	}
};
template< typename T >
...
auto TOSDB_GetStreamSnapshot( std::string id, std::string sItem, TOS_Topics::TOPICS tTopic, long end, long beg ) 
	-> typename std::conditional< b, std::pair< std::vector< T >, std::vector< DateTimeStamp> >, std::vector< T > >::type
{ /* this should be cleaned up; bound to be a type/cast issue in here somewhere */
	size_type bSize;
	long diff;	
	long long minDiff;
	const TOSDBlock *db;
	...
	rGuardTy _lock_(*globalMutex);
	db = GetBlockOrThrow( id );	
	dat = db->block->raw_stream_ptr(sItem, tTopic); // get stream size instead
	bSize = (size_type)(dat->bound_size());
	if( end < 0 ) end += bSize;
	if( beg < 0 ) beg += bSize;
	diff = end - beg;
	if( diff < 0 )
		throw std::invalid_argument(" TOSDB_GetStreamSnapshot(...): invalid indexes( end < beg ) ");
	if( (minDiff = (long long)std::min< size_t >( (size_t)end + 1, dat->size() ) - beg) < 0 )
		minDiff = 0;
	try
	{		
	...
	{
		throw TOSDB_data_stream_error(e, 
			"tosdb_data_stream error caught and encapsulated in TOSDB_GetStreamSnapshot<T,b>()" 
			);
	}
	catch( ... )
	{
...
template< typename T > int 
TOSDB_GetStreamSnapshot_( LPCSTR id,LPCSTR sItem, TOS_Topics::TOPICS tTopic, T* dest, size_type arrLen, pDateTimeStamp datetime, long end, long beg )
{
	const TOSDBlock *db;
	TOSDB_RawDataBlock::stream_const_ptr_type dat;
...
template< typename T > int 
TOSDB_GetStreamSnapshot_( LPCSTR id,LPCSTR sItem, LPCSTR sTopic, T* dest, size_type arrLen, pDateTimeStamp datetime, long end, long beg )
{	
	if( !ChceckStringLength( sTopic ) ) // let this go thru std::string ?
		return -1;		
	return TOSDB_GetStreamSnapshot_( id, sItem, GetTopicEnum( sTopic ), dest, arrLen, datetime,end,beg);
}

int
TOSDB_GetStreamSnapshotDoubles( LPCSTR id,LPCSTR sItem, LPCSTR sTopic, ext_price_type* dest, size_type arrLen, pDateTimeStamp datetime, long end, long beg)
{
	return TOSDB_GetStreamSnapshot_( id, sItem, sTopic, dest, arrLen, datetime,end,beg);
}

int
TOSDB_GetStreamSnapshotFloats( LPCSTR id, LPCSTR sItem, LPCSTR sTopic, def_price_type* dest, size_type arrLen, pDateTimeStamp datetime, long end, long beg)
{
	return TOSDB_GetStreamSnapshot_( id, sItem, sTopic, dest, arrLen, datetime,end,beg);
}

int
TOSDB_GetStreamSnapshotLongLongs( LPCSTR id, LPCSTR sItem, LPCSTR sTopic, ext_size_type* dest, size_type arrLen, pDateTimeStamp datetime, long end, long beg)
{
	return TOSDB_GetStreamSnapshot_( id, sItem, sTopic, dest, arrLen, datetime,end,beg);	
}

int
TOSDB_GetStreamSnapshotLongs( LPCSTR id, LPCSTR sItem, LPCSTR sTopic, def_size_type* dest, size_type arrLen, pDateTimeStamp datetime, long end, long beg)
{
	return TOSDB_GetStreamSnapshot_( id, sItem, sTopic, dest, arrLen, datetime,end,beg);	
}

int
TOSDB_GetStreamSnapshotStrings( LPCSTR id, LPCSTR sItem, LPCSTR sTopic, LPSTR* dest, size_type arrLen, size_type strLen, pDateTimeStamp datetime, long end, long beg)
{
	const TOSDBlock *db;
	TOSDB_RawDataBlock::stream_const_ptr_type dat;
...
template< typename T> int 
TOSDB_GetItemFrame_( LPCSTR id, TOS_Topics::TOPICS tTopic, T* dest, size_type arrLen, LPSTR* dest2, size_type strLen2, pDateTimeStamp datetime)
{
	const TOSDBlock *db;
	int errVal = 0;
	if( !ChceckIDLength(id) )
	...
	try
	{
		...
		if( datetime )
		{
			...
			generic_dts_map_type tmpMDT = db->block->pair_map_of_frame_items(tTopic);
			bIter = tmpMDT.cbegin();
			eIter = tmpMDT.cend();
			if( !dest2 )
			{			
				for( size_type i = 0; (i < arrLen) && (bIter != eIter); ++bIter, ++i )
				{
...
template< typename T> int 
TOSDB_GetItemFrame_( LPCSTR id, LPCSTR sTopic, T* dest, size_type arrLen, LPSTR* dest2, size_type strLen2, pDateTimeStamp datetime)
{	
	if( !ChceckStringLength( sTopic ) )
		return -1;		
	return TOSDB_GetItemFrame_( id, GetTopicEnum( sTopic ), dest, arrLen, dest2, strLen2, datetime);	
}

int
TOSDB_GetItemFrameDoubles( LPCSTR id, LPCSTR sTopic, ext_price_type* dest, size_type arrLen, LPSTR* labelDest, size_type labelStrLen, pDateTimeStamp datetime )
{
	return TOSDB_GetItemFrame_( id, sTopic, dest, arrLen, labelDest, labelStrLen, datetime);		
}

int
TOSDB_GetItemFrameFloats( LPCSTR id, LPCSTR sTopic, def_price_type* dest, size_type arrLen, LPSTR* labelDest, size_type labelStrLen, pDateTimeStamp datetime )
{
	return TOSDB_GetItemFrame_( id, sTopic, dest, arrLen, labelDest, labelStrLen, datetime);			
}

int 
TOSDB_GetItemFrameLongLongs( LPCSTR id, LPCSTR sTopic, ext_size_type* dest, size_type arrLen, LPSTR* labelDest, size_type labelStrLen, pDateTimeStamp datetime)
{
	return TOSDB_GetItemFrame_( id, sTopic, dest, arrLen, labelDest, labelStrLen, datetime);			
}

int 
TOSDB_GetItemFrameLongs( LPCSTR id, LPCSTR sTopic, def_size_type* dest, size_type arrLen,LPSTR* labelDest, size_type labelStrLen, pDateTimeStamp datetime )
{
	return TOSDB_GetItemFrame_( id, sTopic, dest, arrLen, labelDest, labelStrLen, datetime);			
}

int
TOSDB_GetItemFrameStrings( LPCSTR id, LPCSTR sTopic, LPSTR* dest,  size_type arrLen, size_type strLen, LPSTR* labelDest, size_type labelStrLen, pDateTimeStamp datetime )
{	
	const TOSDBlock *db;
	TOS_Topics::TOPICS tTopic;
	int errVal = 0;
			...
			{				
				for( size_type i = 0; (i < arrLen) && (bIter != eIter); ++bIter, ++i )
				{
					...
					if( errVal = strcpy_s(
							dest[i],
							strLen, 
							( bIter->second.first ).as_string().c_str() 
							)
						) return errVal;					
				}
			}
...
int 
TOSDB_GetTopicFrameStrings( LPCSTR id, LPCSTR sItem, LPSTR* dest, size_type arrLen, size_type strLen, LPSTR* labelDest, size_type labelStrLen, pDateTimeStamp datetime)
{	
	const TOSDBlock *db;
	int errVal = 0;
	if( !ChceckIDLength( id ) || !ChceckStringLength(sItem) )
	...
	try
	{
		...
		{
			generic_dts_map_type::const_iterator bIter, eIter;				
			generic_dts_map_type tmpMDT = db->block->pair_map_of_frame_topics(sItem);
			bIter = tmpMDT.cbegin();
			eIter = tmpMDT.cend();
			if( !labelDest )
			{				
...
template<> generic_dts_matrix_type
...
{
	out<<dts.ctime_struct.tm_mon+1<<'/'<<dts.ctime_struct.tm_mday<<'/'<<
		dts.ctime_struct.tm_year+1900<<' '<<dts.ctime_struct.tm_hour<<':'<<
		dts.ctime_struct.tm_min<<':'<<dts.ctime_struct.tm_sec<<':'<<dts.micro_second;
	return out;
}
...
operator<<(std::ostream& out, const generic_dts_vectors_type &vecs)
{
	...
	for( ; iterF != vecs.first.cend() && iterS != vecs.second.cend(); iterF++, iterS++ )
...
template< typename T > 
std::ostream& operator<<(std::ostream& out, const std::pair<T,DateTimeStamp>& pair )
{
...
template< typename T > 
std::ostream& operator<<(std::ostream& out, const std::pair<std::vector<T>,dts_vector_type>& vecs)
{
	auto iterF = vecs.first.cbegin();
	auto iterS = vecs.second.cbegin();
	for( ; iterF != vecs.first.cend() && iterS != vecs.second.cend(); iterF++, iterS++ )
