...
str_set_type TOSDB_GetTopicNames(std::string id)
{
  ...
  return str_set_type(db->block->topics(), 
                      [&](TOS_Topics::TOPICS t){ return TOS_Topics::map[t]; });
  /* --- CRITICAL SECTION --- */
}
...
str_set_type TOSDB_GetPreCachedTopicNames(std::string id)
{
  ...
  return str_set_type(db->topic_precache, 
                      [=](TOS_Topics::TOPICS top){return TOS_Topics::map[top];});
  /* --- CRITICAL SECTION --- */
}
...
struct GRetType<T, false>{
  T operator()(T val, DateTimeStamp dts) { return val; }
};
...
auto TOSDB_GetStreamSnapshot(std::string id, 
  ...
  -> typename std::conditional<b, 
     std::pair<std::vector<T>,std::vector<DateTimeStamp>>, std::vector<T>>::type
{ /* this should be cleaned up; bound to be a type/cast issue in here */
  ...
  diff = end - beg;
  if(diff < 0)
    throw std::invalid_argument(" TOSDB_GetStreamSnapshot(): end < beg ");
        ...
        for(size_type i = 0; 
           ...
           ++b_iter, ++i)
          {
            ...
            err = strcpy_s(dest[i], str_len, 
                           (b_iter->second.first).as_string().c_str());
            if(err) 
              return err;          
          }
      }
...
template<> 
generic_dts_map_type TOSDB_GetTopicFrame<true>(std::string id,std::string sItem)
{
  const TOSDBlock *db;
...
template<> generic_matrix_type TOSDB_GetTotalFrame<false>(std::string id)
{
  const TOSDBlock *db;
...
template<> generic_dts_matrix_type TOSDB_GetTotalFrame<true>(std::string id)
{
  const TOSDBlock *db;
...
template<typename T> 
std::ostream& operator<<(std::ostream& out, 
                         const std::pair<std::vector<T>,dts_vector_type>& vecs)
{
  auto f_iter = vecs.first.cbegin();
  auto s_iter = vecs.second.cbegin();
  for( ; 
