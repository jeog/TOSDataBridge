...
{   
    ...
    auto free_heap_end = _free_heap.cend();
    auto foundR = std::find_if( // look for contiguous free blocks to right 
                    _free_heap.cbegin(),
                    free_heap_end,
                    [&]( heapPairTy elem )
                        {
    ...
    auto foundL = std::find_if( // look for contiguous free blocks to left 
                    _free_heap.cbegin(),
                    free_heap_end,
                    [&]( heapPairTy elem )
                        {
    ...
    if( (foundR != free_heap_end) && (foundL != free_heap_end) ){
        _free_heap.insert( heapPairTy( foundL->first + HEADER + head + foundR->first, foundL->second ) );
        _free_heap.erase( foundR );
        _free_heap.erase( foundL );
    } else if( foundR != free_heap_end){
        _free_heap.insert( heapPairTy( head + HEADER + foundR->first, pHead ) );
        _free_heap.erase( foundR );
    } else if( foundL != free_heap_end){
        _free_heap.insert( heapPairTy( foundL->first + HEADER + head, foundL->second ) );
        _free_heap.erase( foundL );
    } else
...
void* DynamicIPCBase::_allocate( size_type sz ) const
{
    ...
    return CallNamedPipe( 
            _intrnlPipeStr.c_str(),
            (void*)outBuf, sizeof(outBuf),
            (void*)&(retOff), sizeof(retOff),
            &bRead,
            TOSDB_DEF_TIMEOUT ) == TRUE ? ptr(retOff) : nullptr;
}
...
bool DynamicIPCBase::_deallocate( void* start ) const
{
    ...
    return CallNamedPipe( 
            ...
            &bRead,
            TOSDB_DEF_TIMEOUT ) == TRUE ? (retVal == TRUE ? true : false) : false;
}
...
int DynamicIPCMaster::grab_pipe()
{
    ...
    {
        TOSDB_LogEx("IPC-Master", "WaitForSingleObject() [_mtx] timed out in grab_pipe()", GetLastError() );        
        CloseHandle( _mtx );
        return -1;    
    }
    if( !WaitNamedPipe( _pipeStr.c_str(), 0 ) )    // ADD A TIMEOUT FOR THIS ???
    {
        TOSDB_LogEx("IPC-Master", "WaitNamedPipe() failed in grab_pipe()", GetLastError() );
        ReleaseMutex( _mtx );
        CloseHandle( _mtx );
        return -1;    
    }    
...
{    
    ...
    {            
        TOSDB_LogEx("IPC-Master", "grab_pipe() in try_for_slave() failed", GetLastError() );
        disconnect( 0 );        
        return false;
    }
    // WE HAD A RACE CONDITION HERE WHERE THE SLAVE MAY DELETE THE SHARED MEM FIRST
    // SHOULD BE O.K. NOW 
    _fMapHndl = OpenFileMapping( FILE_MAP_WRITE, 0, _shemStr.c_str() ); 
    if( !_fMapHndl) 
    {
        TOSDB_LogEx("IPC-Master", "OpenFileMapping() in try_for_slave() failed", GetLastError() );
        release_pipe();
        disconnect( 1 );        
        return false;
    }
    ...
    {
        TOSDB_LogEx("IPC-Master", "MapViewOfFile() in try_for_slave() failed", GetLastError() );
        release_pipe();
        disconnect( 2 );
        return false;
    }    
    // release our exclusive connection AFTER FileMapping calls to avoid race with slave 
    release_pipe();    
    return true;
}
    ...
    case 3:
        if(_mMapAddr)                    
            if( !UnmapViewOfFile( _mMapAddr ))
                TOSDB_LogEx("IPC-Master", "UnmapViewOfFile in DynamicIPCMaster::disconnect() failed", GetLastError());        
        _mMapAddr = NULL;                    
    case 2:    
...
DynamicIPCSlave::DynamicIPCSlave( std::string name, int sz )
    ...
    { 
        ...
        {            
            ...
            _everyoneSIDs[(Securable)i] = std::move( SmartBuffer<void>(SECURITY_MAX_SID_SIZE) );
            _everyoneACLs[(Securable)i] = std::move( SmartBuffer<ACL>(ACL_SIZE) );        
        }                                 
        _set_security();                                        
        if( !(_mtx = CreateMutex( &_secAttr[MUTEX1], FALSE, KMUTEX_NAME )) )    
            TOSDB_LogEx("IPC-Slave", "CreateMutex() in slave constructor failed", GetLastError() );        
        _intrnlPipeHndl = CreateNamedPipe( 
                            _intrnlPipeStr.c_str(), 
                            PIPE_ACCESS_DUPLEX,
        ...
        if(!_intrnlPipeHndl)
            TOSDB_LogEx("IPC-Slave", "CreateNamedPipe()[internal] in slave constructor failed", GetLastError() );
        _fMapHndl = CreateFileMapping( /* get handle to page file */
        ...
        if(!_fMapHndl)
            TOSDB_LogEx("IPC-Slave", "CreateFileMapping() in slave constructor failed", GetLastError());    
        if( !(_mMapAddr = MapViewOfFile( _fMapHndl, FILE_MAP_WRITE, 0, 0, 0 )))
            TOSDB_LogEx("IPC-Slave", "MapViewOfFile() in slave constructor failed", GetLastError() );    
        /* consturct our simple heap for managing the mem-map's allocatable space */
        _pHeap = std::unique_ptr<ExplicitHeap>(new ExplicitHeap(_mMapAddr,_mMapSz));        
...
DynamicIPCSlave::~DynamicIPCSlave()
    {    
        ...
        CloseHandle( _mtx );             
        if( !UnmapViewOfFile( _mMapAddr ))
            TOSDB_LogEx("IPC-Slave", "UnmapViewOfFile() in slave destructor failed", GetLastError() );    
        CloseHandle( _fMapHndl );            
    ...
    while( _allocLoopF )
    {
        ...
        case ALLOC:
            {                
                ...
                args[1] = offset(start);
                WriteFile( _intrnlPipeHndl, (void*)&args[1], sizeof(args[1]), &bDone, NULL );                
            }
            break;
        case DEALLOC:
            {                
        ...
        case PING:
            {
                if( !_mMapAddr || !_fMapHndl )
                    --(args[1]); /* if we don't have our mem-map send back a differen val */
                WriteFile( _intrnlPipeHndl, (void*)&args[1], sizeof(args[1]), &bDone, NULL );
            }
            break;            
        default:
...
{    
    ...
    if( !(this->send(_mMapSz)) )
        TOSDB_LogEx("IPC-Slave", "->send() failed in wait_for_master()", GetLastError() );    
    return true;
}
...
{    
    ...
    if( memcpy_s( _everyoneSIDs[SHEM1].get(), SECURITY_MAX_SID_SIZE, everyoneSID.get(), SECURITY_MAX_SID_SIZE ) ||
        memcpy_s( _everyoneSIDs[PIPE1].get(), SECURITY_MAX_SID_SIZE, everyoneSID.get(), SECURITY_MAX_SID_SIZE ) ||
        memcpy_s( _everyoneSIDs[MUTEX1].get(), SECURITY_MAX_SID_SIZE, everyoneSID.get(), SECURITY_MAX_SID_SIZE ) ) return -2;

    if( !InitializeSecurityDescriptor( &_secDesc[SHEM1], SECURITY_DESCRIPTOR_REVISION ) ||
        !InitializeSecurityDescriptor( &_secDesc[PIPE1], SECURITY_DESCRIPTOR_REVISION ) ||
        !InitializeSecurityDescriptor( &_secDesc[MUTEX1], SECURITY_DESCRIPTOR_REVISION ) ) return -3; 

    if( !SetSecurityDescriptorGroup( &_secDesc[SHEM1], _everyoneSIDs[SHEM1].get(), FALSE ) ||
        !SetSecurityDescriptorGroup( &_secDesc[PIPE1], _everyoneSIDs[PIPE1].get(), FALSE ) ||
        !SetSecurityDescriptorGroup( &_secDesc[MUTEX1], _everyoneSIDs[MUTEX1].get(), FALSE ) ) return -4;
    ...
    if( !InitializeAcl( _everyoneACLs[SHEM1].get(), ACL_SIZE, ACL_REVISION ) ||
        ...
        !InitializeAcl( _everyoneACLs[MUTEX1].get(), ACL_SIZE, ACL_REVISION ) ) return -5; 
                                                                    
    if( !AddAccessAllowedAce( _everyoneACLs[SHEM1].get(), ACL_REVISION, FILE_MAP_WRITE, _everyoneSIDs[SHEM1].get() ) ||
        !AddAccessAllowedAce( _everyoneACLs[PIPE1].get(), ACL_REVISION, FILE_GENERIC_WRITE, _everyoneSIDs[PIPE1].get() ) ||
        !AddAccessAllowedAce( _everyoneACLs[PIPE1].get(), ACL_REVISION, FILE_GENERIC_READ, _everyoneSIDs[PIPE1].get() ) ||
        !AddAccessAllowedAce( _everyoneACLs[MUTEX1].get(), ACL_REVISION, SYNCHRONIZE, _everyoneSIDs[MUTEX1].get() ) ) return -6;

    if( !SetSecurityDescriptorDacl( &_secDesc[SHEM1], TRUE, _everyoneACLs[SHEM1].get(), FALSE ) ||
        !SetSecurityDescriptorDacl( &_secDesc[PIPE1], TRUE, _everyoneACLs[PIPE1].get(), FALSE ) ||
        !SetSecurityDescriptorDacl( &_secDesc[MUTEX1], TRUE, _everyoneACLs[MUTEX1].get(), FALSE ) ) return -7;
