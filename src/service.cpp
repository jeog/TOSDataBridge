...
void UpdateStatus( int status, int check_point )
{
	...
	if( !SetServiceStatus( hServiceStatus, &serviceStatus ) )
	{
		...
		if( !SetServiceStatus( hServiceStatus, &serviceStatus) )
		{
			TOSDB_LogH("SERVICE-UPDATE","fatal error handling service error, terminating process");
...
int SendMsgWaitForResponse( const unsigned int msg )
{
	...
	{
		globalIPCMaster = 
			std::unique_ptr<DynamicIPCMaster>(std::move( new DynamicIPCMaster( TOSDB_COMM_CHANNEL )));
		...
		while( globalIPCMaster->grab_pipe() <= 0 )
		{
			...
			{
				TOSDB_LogH("IPC","SendMsgWaitForResponse timed out trying to grab pipe");
				return -1;
			}
		}
	}	
		...
		{			
			...
			{	/* get it to continue silently */				
				if( SendMsgWaitForResponse( TOSDB_SIG_CONTINUE ) != TOSDB_SIG_GOOD)
					TOSDB_Log("SERVICE-ADMIN", "error trying to resume paused thread to stop it");				
			}			
	...
	case SERVICE_CONTROL_PAUSE:
		{		
			...
			if( SendMsgWaitForResponse( TOSDB_SIG_PAUSE ) == TOSDB_SIG_GOOD )
			{
				...
				TOSDB_LogH("SERVICE-ADMIN","core process failed to confirm pause request");						
		}
		break;
...
{			
	...
	{
		TOSDB_LogEx("SpawnRestrictedProcess", "(1) failed to open token handle", GetLastError());
		return false; 	
	}

	if( !DuplicateTokenEx( hToken, 0, NULL, SecurityImpersonation, TokenPrimary, &hChildToken ) )
	{		
		if( hToken ) CloseHandle(hToken);  
		TOSDB_LogEx("SpawnRestrictedProcess", "(2) failed to duplicate token", GetLastError());
		return false; 
	} 	
	/* try to drop our integrity level from System */
	if( LookupAccountName(NULL, integrityLevel.c_str() ,sidBuffer.get(), &sidSz, domBuffer.get(), &domSz, &sidUseDummy) )
	{ 	
		TOKEN_MANDATORY_LABEL tl;
		tl.Label.Attributes = SE_GROUP_INTEGRITY;
		tl.Label.Sid = sidBuffer.get();
		DWORD sz = sizeof(TOKEN_MANDATORY_LABEL) + GetLengthSid( sidBuffer.get() );
		if( !SetTokenInformation( hChildToken, TokenIntegrityLevel, &tl, sz ) )
			TOSDB_LogEx("SpawnRestrictedProcess", "(3) failed to set Integrity Level", GetLastError());
	}
	else 
		TOSDB_LogEx("SpawnRestrictedProcess", "(3) failed to lookup Mandatory Level group", GetLastError());
	
	sessionID = WTSGetActiveConsoleSessionId();		
	/* try to get out of Session 0 isolation if we need to */
	if( isService && !SetTokenInformation( hChildToken, TokenSessionId, &sessionID, sizeof(DWORD) ) )	
		TOSDB_LogEx("SpawnRestrictedProcess", "(4) failed to set session ID", GetLastError());	
	else
	{ /* remove privileges */
		DWORD retLen;
		...
		GetTokenInformation( hChildToken, TokenPrivileges, privBuf.get(), retLen, &retLen);				
		pTpriv = privBuf.get();
		...
		if( !AdjustTokenPrivileges( hChildToken, FALSE, privBuf.get(), 0, NULL, NULL ) )
			TOSDB_LogEx("SpawnRestrictedProcess", "(5) failed to adjust token privileges", GetLastError());	
		GetStartupInfo(&stupInfo);
		/* try to create the process with the new token */		
		if( !CreateProcessAsUser( hChildToken, coreProcPath.c_str(), coreProcCmd, NULL, NULL, FALSE, 0, NULL, NULL, &stupInfo, &coreProcInfo) )			
			TOSDB_LogEx("SpawnRestrictedProcess", "(6) failed to create core process", GetLastError()); 	
		else 
			retVal = true; 
	}
			
	if( hToken ) CloseHandle(hToken);
	if( hChildToken ) CloseHandle(hChildToken); 
	return retVal;
}	
};
...
void WINAPI ServiceMain( DWORD argc, LPSTR argv[] )
{
	...
	if( !SpawnRestrictedProcess() )
		TOSDB_LogH("SERVICE-ADMIN", std::string("failed to fork ").append(CORE_PROC_NAME).c_str());		
	else
	{
		UpdateStatus( SERVICE_RUNNING, -1 );
		TOSDB_Log("SERVICE-CNTRL","SERVICE_RUNNING");
...
{			
	...
	if( arg1 == "--noservice" || arg2 == "--noservice" )
	{
		...
		isService = false;
		TOSDB_Log("STARTUP", "tos-databridge-engine.exe starting - NOT A WINDOWS SERVICE");
	...
	else 
	{
		...
		TOSDB_Log("STARTUP", "tos-databridge-engine.exe starting - WINDOWS SERVICE");
