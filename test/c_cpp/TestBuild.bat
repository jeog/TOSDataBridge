...
IF "%1"=="x86" (
    ...
    set "OURlibstub=!RELdir!\Win32\tos-databridge-!VERSION!-x86.lib"
    set "OURdllname=tos-databridge-!VERSION!-x86.dll"
    ...
    IF "%1"=="x64" (
        ...
        set "OURlibstub=!RELdir!\x64\tos-databridge-!VERSION!-x64.lib"
        set "OURdllname=tos-databridge-!VERSION!-x64.dll"
...
IF NOT EXIST %OURdllname% ( 
    COPY %OURdllpath% . 
)
IF %ERRORLEVEL% NEQ 0 ( 
    echo fatal: error copying %OURdllname%
    EXIT /B 1 
)

IF NOT EXIST %OURdll2name% ( 
    COPY %OURdll2path% . 
)
IF %ERRORLEVEL% NEQ 0 ( 
    echo fatal: error copying %OURdll2name%
    EXIT /B 1 
)
...
:usage_warn
echo.
echo Usage: %THISbin%  [x86^|x64]  [C^|CPP]  [VS_TOOLS_NUM]
echo.
echo        [x86^|x64]      - build
echo        [C^|CPP]        - language
