...
const TOS_Topics::topic_map_type& TOS_Topics::globalTopicMap 
	= TOS_Topics::_globalTopicMap;
...
static struct PyModuleDef _tosdb 
	= { PyModuleDef_HEAD_INIT, "_tosdb", NULL, -1, NULL };
...
{	
	...
	PyObject* topicObj = PyTuple_New( TOS_Topics::globalTopicMap.size() ); 
	TOS_Topics::topic_map_type::const_iterator1_type cbIter = TOS_Topics::globalTopicMap.cbegin();
	int count = 0;
	do
		PyTuple_SET_ITEM( topicObj, count++, PyUnicode_FromString(cbIter->second.c_str()) );	
